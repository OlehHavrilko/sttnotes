...
## Offline model setup
Open **Models** to install the current official whisper.cpp Windows x64 ZIP and supported Whisper weights, or choose a local folder containing `whisper-cli.exe` and model files (`.bin`, `.gguf`, `.pt`, `.onnx`, or `.safetensors`). Downloads follow redirects, write atomically, report progress, and verify the extracted executable; failed operations show an actionable error and are never marked installed. Network access is only initiated by an explicit download click. A fully offline first run can still use models imported from a local directory.
...
The Models dialog installs supported Whisper model files into the app's local `models` directory and exposes the actual detected engine/model state. Downloads are only model installation; transcription is always performed locally. A compatible `whisper-cli.exe` must be installed or selected because model weights alone are not an executable engine. Settings persist audio format, language, push-to-talk state/key, and reset controls. Hold the configured key (Space by default) while outside text inputs to record; clicking Record remains available.
